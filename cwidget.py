...
class LoginScreen(widget.QFrame):
    def __init__(self, parent: widget.QWidget):
        ...
        self.loadingScreen = LoadingScreen(self, self.postLoadingProcess)
        ...
        self.loginContainer = Container(self)
        ...
        self.signInButton = Button(self, text="Continue")
        self.signInButton.clicked.connect(self.signIn)
        self.signInPageLayout.addWidget(self.signInButton, 4)

        self.createAccountButton = LinkButton(self, text="Create Account")
...
class Label(widget.QLabel):
    def __init__(self, parent: widget.QWidget, text: str):
        super().__init__(parent)
        self.setText(text)
...
class TextEntry(widget.QLineEdit):
    def __init__(self, parent: widget.QWidget, placeholder: str):
        super().__init__(parent)
...
class TitleBar(widget.QFrame):
    def __init__(self, parent: widget.QWidget):
        super().__init__(parent)
        self.setStyleSheet(f"background-color: rgb{settings.DEFAULT_COLOR.getRgb()};"
                           "color: white;")
        self.setFixedHeight(75 * settings.DPI)

        layout = widget.QHBoxLayout()
        layout.setContentsMargins(15 * settings.DPI, 5 * settings.DPI, 15 * settings.DPI, 5 * settings.DPI)
        self.setLayout(layout)

        # Title text
        titleLabel = widget.QLabel(parent=self, text="Junktion File Share")
        titleLabel.setAlignment(core.Qt.AlignVCenter | core.Qt.AlignLeft)
        titleLabel.setStyleSheet(f"font-size: {28 * settings.DPI}px;"
                                 f"font-family: Arial;"
                                 "font-weight: bold;")
        layout.addWidget(titleLabel)

        # Add username text
        usernameLabel = widget.QLabel(parent=self, text="Username")
        usernameLabel.setAlignment(core.Qt.AlignVCenter | core.Qt.AlignRight)
        usernameLabel.setStyleSheet(f"font-size: {25 * settings.DPI}px;"
                                    f"font-family: Arial;"
                                    "font-weight: bold;")
        layout.addWidget(usernameLabel)

        # Add user profile picture
        userProfilePicture = widget.QFrame(parent=self)
        userProfilePicture.setFixedSize(60 * settings.DPI, 60 * settings.DPI)
        userProfilePicture.setStyleSheet("background-color: rgb(255, 0, 0);")
        layout.addWidget(userProfilePicture)


class NavigationBar(widget.QFrame):
    def __init__(self, parent: widget.QWidget):
        super().__init__(parent)
        layout = widget.QHBoxLayout()
...
class DirectoryBar(widget.QFrame):
    def __init__(self, parent: widget.QWidget):
        ...
        self.refreshButton.setStyleSheet("QPushButton {"
                                         ...
                                         f"background-color: rgb{settings.DEFAULT_COLOR.getRgb()};"
                                         "color: white;"
                                         f"border-top-right-radius: {6 * settings.DPI}px;"
                                         f"border-bottom-right-radius: {6 * settings.DPI}px;"
                                         f"font-size: {20 * settings.DPI}px;"
                                         "}"
                                         "QPushButton::hover"
                                         "{"
                                         ...
                                         f"background-color: rgb{settings.DEFAULT_COLOR_PRESSED.getRgb()};"
                                         "}")
        refreshButtonIcon = gui.QIcon(PixmapImage(filename="resources/refresh.png").scaledToHeight(45 * settings.DPI))
        self.refreshButton.setIcon(refreshButtonIcon)
        self.refreshButton.clicked.connect(self.startRefresh)
        layout.addWidget(self.refreshButton)

    def startRefresh(self) -> None:
        self.window().threadManager.start(self.refresh)
